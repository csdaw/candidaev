...
# filter for proteins quantified in min 1/3 reps of 10231 EV
atcc1_ev <- filter_na(atcc_norm, 
                      op = "<=", 
                      pat = "A10231_EV", 
                      val = 2)

# filter for proteins quantified in min 1/3 reps of 10231 WCL
atcc1_w <- filter_na(atcc_norm, 
                     op = "<=", 
                     pat = "A10231_W", 
                     val = 2)

# filter for proteins quantified in min 1/3 reps of 90028 EV
atcc9_ev <- filter_na(atcc_norm, 
                      op = "<=", 
                      pat = "A90028_EV", 
                      val = 2)

# filter for proteins quantified in min 1/3 reps of 90028 WCL
...
that appears in the Venn diagram has been quantified in at least 2/3 replicates 
of one condition. 
...
Of `r nrow(atcc_norm)` proteins, 641 (41.7%) are quantified in at least 1/3 
replicates from all 4 conditions **and** are quantified in at least 2/3 
replicates of 1 condition.
...
# get top 10 most abundant proteins exclusive to 10231 EVs
...
atcc_overall <- get_results(efit = atcc_efit, 
                            mat = atcc_de, 
                            p_val = 0.01, 
                            lfc = 0, 
                            type = "overall")
...
atcc_ind <- get_results(efit = atcc_efit, 
                        mat = atcc_de, 
                        p_val = 0.01, 
                        lfc = 0, 
                        type = "individual")
...
proteins that are significantly different across all four sample groups; 
...
the `logFC` data columns, and a logical significance column. Then the 
data.frame is filtered for significant proteins, the UniProt accession column 
is converted to rownames, the significance column is dropped, and the 
data.frame is converted to a matrix.
