...
knitr::kable(wcl_longlist[, -10], 
             ...
             linesep = "", 
             escape = FALSE) %>% 
  row_spec(row = c(23, 30, 34, 35, 44, 51), underline = TRUE) %>% 
  add_header_above(c(" " = 2, "log\\\\textsubscript{2}(fold change) EV vs WCL" = 4, " " = 3), escape = FALSE) %>% 
  ...
  pack_rows("Cell wall, cell surface, fungal biofilm matrix", 42, 50) %>% 
  pack_rows("Cytoskeleton", 51, 51) %>% 
  pack_rows("Translation preinitiation complex", 52, 52) %>% 
