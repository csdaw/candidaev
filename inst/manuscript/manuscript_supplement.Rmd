...
title: | 
   **Supplement to: Protein markers for *Candida albicans* EVs include
   claudin-like Sur7 family proteins and Rho GTPases**
author: | 
...
```{r figureS1, fig.cap=fs1_cap, fig.width=5.45, fig.height=6}
# construct figure 1
...
wcl_longlist2 <- wcl_longlist %>% 
  mutate(Function = function_desc2, 
         VDM = c(rep("", 40)), 
         pack_row = c(rep(4, 2), 3, rep(2, 2), 7, rep(1, 2), rep(7, 2), 1, 
                      rep(2, 4), 7, 3, 2, 5, 1, 
                      2, 6, 2, 1, 7, rep(1, 2), 2, 1, 5, 1, 
                      1, 6, 2, 1, rep(8, 2), rep(1, 3))) %>% 
  arrange(pack_row)
```
...
tabs2_cap <- "\\textbf{Candidate negative protein markers for 
              ...
              EV markers according to the criteria depicted in Figure S1."

# draw table S2
knitr::kable(wcl_longlist2[, -10], 
             caption = tabs2_cap, 
             format = "latex", 
             booktabs = TRUE, 
             longtable = TRUE, 
             linesep = "", 
             escape = FALSE) %>% 
  row_spec(row = c(20, 25, 29, 36), underline = TRUE) %>% 
  add_header_above(c(" " = 2, 
                     "log\\\\textsubscript{2}(fold change) EV vs WCL" = 4, 
                     " " = 3), escape = FALSE) %>% 
  kable_styling(font_size = 8, 
                latex_options = c("HOLD_position")) %>% 
  pack_rows("Cytoplasm", 1, 14) %>% 
  pack_rows("Mitochondria", 15, 25) %>% 
  pack_rows("Nucleus", 26, 27) %>% 
  pack_rows("Vacuole", 28, 29) %>% 
  pack_rows("Peroxisome", 30, 31) %>% 
  pack_rows("Proteasome", 32, 33) %>% 
  pack_rows("Cell wall, cell surface, fungal biofilm matrix", 34, 38) %>% 
  pack_rows("Ribosome", 39, 40) %>% 
