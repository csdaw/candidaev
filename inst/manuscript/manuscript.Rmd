...
marker_longlist2 <- marker_longlist %>% 
  mutate(Function = function_desc1, 
         pack_row = c(4, rep(1, 3), rep(4, 2), 1, 2, rep(1, 6), 3, rep(4, 2),
                      1, rep(2, 3), 1, rep(4, 2), 1, rep(4, 3), 1, 3, 
                      rep(1, 3), 4, 1, 2, 1, 2, 4)) %>% 
  arrange(pack_row) %>% 
  mutate(VDS = c(rep("", 11), "Y", rep("", 7), "Y", rep("", 7), 
                 rep("Y", 7), "", rep("Y", 4)))

# fix number of TM domains for CDR1;CDR2
marker_longlist2[2, 7] <- "12;12" 
...
# fix SUR7 entries
marker_longlist2[18, 7] <- "4"
marker_longlist2[18, 8] <- ""

# Change name of orf19.6741 to Evp1
marker_longlist2[12, 1] <- paste0("EVP1",
...
knitr::kable(marker_longlist2[ ,-9],
             ...
             linesep = "", 
             escape = FALSE)  %>% 
  row_spec(row = c(1, 5, 11, 12, 15:19, 21:25, 35), underline = TRUE) %>% 
  add_header_above(c(" " = 2, 
                     "log\\\\textsubscript{2}(fold change) EV vs WCL" = 4, 
                     " " = 2), escape = FALSE) %>% 
  kable_styling(font_size = 8) %>% 
  pack_rows("Plasma membrane", 1, 19) %>% 
  pack_rows("Golgi, ER, vacuole", 20, 25) %>% 
  pack_rows("Mitochondria", 26, 27) %>% 
  pack_rows("Cell wall, cell surface", 28, 39) %>% 
...
f2b2 <- plot_volcano(data = a9_res, 
                     ...
                     legend_params = c("left", 11, 8), 
                     axis_params = c("black", 10)) + 
  scale_colour_manual(labels = c("EV enriched (86)", "WCL enriched (122)", 
                                 "Not significant (761)"),
                      values = unname(ev_pal[c(2,3,1)])) + 
  annotate("text", x = 8, y = 0.5, label = "bold('ATCC90028')", parse = TRUE) + 
  annotate("text", x = 3.5, y = 7, label = "bold('86')", 
           parse = TRUE, colour = unname(ev_pal[2])) + 
  annotate("text", x = -3.5, y = 7, label = "bold('122')", 
           parse = TRUE, colour = unname(ev_pal[3])) + 
  annotate("text", x = 0, y = 6, label = "bold('761')", 
...
f2b3 <- plot_volcano(data = a1_res, 
                     ...
                     legend_params = c("left", 11, 8), 
                     axis_params = c("black", 10)) + 
  scale_colour_manual(labels = c("EV enriched (55)", "WCL enriched (52)", 
                                 "Not significant (567)"),
                      values = unname(ev_pal[c(2,3,1)])) + 
  annotate("text", x = 8, y = 0.5, label = "bold('ATCC10231')", parse = TRUE) + 
  annotate("text", x = 3.5, y = 7, label = "bold('55')", 
           parse = TRUE, colour = unname(ev_pal[2])) + 
  annotate("text", x = -3.5, y = 7, label = "bold('52')", 
           parse = TRUE, colour = unname(ev_pal[3])) + 
  annotate("text", x = 0, y = 6, label = "bold('567')", 
           parse = TRUE, colour = unname(ev_pal[1]))
```
...
f3_a9 <- plot_go_bar(f3_a9_go_table, c(1:3,6, 27,29,32:33, 46,49,51:52), f3_pal) + 
  annotate("text", x = 12, y = 70, label = "bold('ATCC90028')", parse = TRUE)

f3_a1 <- plot_go_bar(f3_a1_go_table, c(1:4, 62,64:66, 86:89), f3_pal) + 
...
f6_cap <- "\\textbf{Identification of common signficantly enriched or exclusive 
           ...
           to all 4 datasets. 39 proteins were identified as potential positive 
           EV protein markers and 40 proteins as potential negative EV protein 
           markers. The data tables underlying Figure 
           \\@ref(fig:figure6) are provided in supplemental File S4. Details of 
           the 39 positive EV protein marker candidates and 40 negative 
...
f7_cap <- "\\textbf{Eighteen best potential \\textit{C. albicans} positive EV 
