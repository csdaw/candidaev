...
First, the `atcc` data.frame is filtered to remove reverse proteins, contaminant 
proteins, and proteins identified by less than 2 unique peptides.
...
2. Returns a matrix containing only: the UniProt accession(s) and LFQ intensities 
   of each protein across the 12 samples. 
...
```{r atccInit2, fig.cap="This is a figure caption. This is the second sentence.", fig.wide=TRUE}
a1a <- plot_frequency2(atcc_lfq) + 
  scale_y_continuous(limits = c(0,600), expand = expand_scale(mult = c(0, 0)))

plot_numbers2(atcc_lfq, atcc_exp) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) + 
  scale_y_continuous(limits = c(0, 2250), expand = expand_scale(mult = c(0, 0))) + 
  labs(fill = "Sample\ntype")
...
```{r markerComparison2, results='hide'}
