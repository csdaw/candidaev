...
f3_cap <- "\\textbf{Differential enrichment analysis of proteins identified in \\textit{C. albicans} 
           ...
           Differential enrichment was performed using the R package \\textit{limma}. Significantly enriched proteins were
           identified using a Benjamini-Hochberg adjusted p-value cutoff of 0.01. Counts of significant and 
           non-significant proteins are indicated on each graph. Proteins with a $\\log_2{FC}$ greater than 
           8 are labelled. Data tables underlying these figures are provided in supplemental File x"
```
...
f4b <- plot_mds(f4b_data, mat_labels = f4b_labels, shape_size = 3) + 
  ...
  scale_fill_manual(breaks = c("A1", "A9", "B", "Y"), 
                    ...
                    guide = "legend") + 
  scale_shape_manual(breaks = c("EV", "WCL"), 
                     values = c(22,24)) + 
  theme_bw() + 
...
```{r figure4, fig.cap='Combined comparison', fig.width=7.1, fig.height=6}
