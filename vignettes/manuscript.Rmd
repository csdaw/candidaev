...
marker_longlist <- marker_longlist %>% 
  mutate(Function = protein_desc, 
         VD = c("Y", rep("",3), rep("Y", 2), "", "Y", rep("", 9), rep("Y", 2), 
...
# TMHMM v2.0 says SUR7 has 4 TM domains
# SignalP v5.0 has a 0.2988 likelihood of a signal peptide and 0.7012 of 'Other'
# fix SUR7 entries
...
```{r markerTable}
tab1_cap <- "\\textbf{43 protein marker candidates for positive identification of \\textit{C. albicans} 
             EVs.} The names, functions, abundances, and certain properties of each of the proteins 
             identified in Figure \\ref{fig:figure6} are given. Proteins with larger 
             log\\textsubscript{2}(fold change) have a greater abundance compared to WCL for a particular strain, whilst
             'ex' indicates a protein is only found in EVs. The 'TM' column records the number of 
             transmembrane domains for each protein, 'SP' indicates whether a protein has a predicted
             signal peptide, and 'VD' shows whether a protein has previously been detected in 
             vesicle-depleted culture media (i.e. it may also be in the soluble secretome); see (ref:table1). 
             Underlined proteins are those identified 
             as ideal candidates for preliminary EV markers according to the criteria depicted in 
...
knitr::kable(marker_longlist[ ,-10],
  ...
  pack_rows("Golgi, ER, secretory pathway", 20, 26) %>% 
  pack_rows("Vacuole", 27, 28) %>% 
  pack_rows("Mitochondria", 29, 31) %>% 
  pack_rows("Cell wall, cell surface", 32, 43) %>% 
  footnote(alphabet = c("Protein localisation was inferred from sequence similarity with \\\\textit{S. cerevisiae} ortholog (see CGD)", 
                        "Protein localisation was predicted using DeepLoc-1.0", 
                        "Presence of transmembrane domains and signal peptide predicted with TMHMM v2.0 and SignalP 5.0 respectively"), 
           escape = FALSE, 
           threeparttable = TRUE)
...
f2_cap <- "\\textbf{Characterisation of EVs from different \\textit{C. albicans} strains and morphologies.} 
           \\textit{A}, Representative TEM images of EVs isolated from 4 different \\textit{C. albicans} cultures.
           Scale bar indicates 500 nm. \\textit{B}, Size distribution of DAY286 biofilm (n = 5), DAY286 yeast (n = 3), 
           ...
           \\textit{C}, Comparison of the ratio of particle concentration (particles/mL)/protein concentration 
           ($\\mu$g/mL) across the four EV sources. Each dot represents 1 biological replicate. Sample means were 
           compared using ANOVA followed by Tukey's HSD \\textit{post-hoc} test. P-values indicating significant 
...
f3_cap <- "\\textbf{Differential enrichment analysis of proteins identified in \\textit{C. albicans} 
           ...
           proteomes from various \\textit{C. albicans} strains; DAY286 yeast (n = 3), DAY286 biofilm (n = 5), 
           ATCC90028 yeast (n = 3), and ATCC10231 yeast (n = 3). Whole cell lysates were prepared from the EV 
           source cells. Proteins depicted were identified in minimum 2/3 (yeast) and 4/5 (biofilm) biological 
           ...
           identified in both EV and WCL (i.e. proteins in venn overlap) using the R package 
           \\textit{limma} [@cox2014; @ritchie2015]. Significantly enriched proteins were identified using a 
           Benjamini-Hochberg adjusted p-value cutoff of 0.01. Counts of significant and 
           non-significant proteins are indicated on each graph. Proteins with a $\\log_2{FC}$ greater than 
           8 are labelled. Data tables underlying the venn diagrams and volcano plots are provided in 
           supplemental Files x and y respectively."
```
...
f4_cap = "\\textbf{Combined comparison of \\textit{C. albicans} EV proteomes.} \\textit{A}, 
          ...
          EV proteins. The online tool FungiFun2 was used to identify enriched biological process (BP), 
          ...
          found in supplemental File x. \\textit{B}, Multidimensional scaling (MDS) plot of each LC-MS/MS
          biological replicate sample shows the sources of variation in the data. Pairwise distance between 
          samples approximately indicates the $\\log_2{FC}$ between samples. Multidimensional scaling 
          was performed using \\texttt{plotMDS} from the R package \\textit{limma} [@ritchie2015]. 
          ...
          \\@ref(fig:figure4)C is provided in supplemental File x."
...
f5_cap <- "\\textbf{Identification of commonly enriched EV proteins across different \\textit{C. albicans} strains
           and morphologies.} Heatmap of proteins (n = 405) identified in all four EV samples. Each column 
           represents a \\textit{C. albicans strain} and each row represents a protein group. The colours
           indicate the $\\log_2{FC}$ of the protein for that particular strain; red is EV enriched, blue is 
           EV depleted, and black is EV exclusive. Unsupervised clustering of rows and columns was performed 
           using Gower's formula via the \\texttt{daisy} function from the R package \\textit{cluster} (ref).
...
f6_cap <- "\\textbf{Identification of common signficantly enriched or exclusive \\textit{C. albicans} 
           EV and WCL proteins.} Proteins identified as significantly enriched or exclusive in Figure 
           \\@ref(fig:figure3) were compared to select those which were common to all 4 datasets. 43 
           proteins were identified as potential positive EV protein markers and 60 
           proteins as potential negative EV protein markers. The data tables underlying Figure 
          \\@ref(fig:figure6) are provided in supplemental File x."
```


\newpage

```{r figure7, fig.cap='4tm domain'}
fig7 <- multi_panel_figure(width = 7.1, height = 6, unit = "inches", columns = 2, rows = 4, 
...
```{r figure8, fig.cap='final candidates'}
plot(mtcars[,1:2])
```
