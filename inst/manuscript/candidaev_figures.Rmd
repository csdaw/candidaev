...
test2 <- aov(Particle_count ~ Strain * bin, data = test)
summary(test2)

TukeyHSD(test2, which = "Strain")
...
# GO term bar plots
f3_y <- plot_go_bar(f3_y_go_table, c(1:4,6, 23,26:28,30, 42,44:46), f3_pal) + 
  annotate("text", x = 14, y = 65, label = "bold('DAY286 Y')", parse = TRUE) + 
  labs(fill = "GO domain") + 
  theme(legend.text = element_text(size = 10), 
        legend.title = element_text(size = 10))

f3_b <- plot_go_bar(f3_b_go_table, c(1:4,6, 54:57,67, 73,75,77,87), f3_pal) + 
  annotate("text", x = 14, y = 60, label = "bold('DAY286 B')", parse = TRUE)

f3_a9 <- plot_go_bar(f3_a9_go_table, c(1:5, 12,14,17,18,24, 26:29), f3_pal) + 
  annotate("text", x = 14, y = 70, label = "bold('ATCC90028')", parse = TRUE)

f3_a1 <- plot_go_bar(f3_a1_go_table, c(1:4,6, 31,33:35,45, 49:52), f3_pal) + 
  annotate("text", x = 14, y = 70, label = "bold('ATCC10231')", parse = TRUE)
...
```{r figure3, fig.width=7.2, fig.height=6, fig.path='figures/', dev=c('pdf', 'postscript', 'tiff'),  dev.args=f3_args, dpi=600}
# draw figure 3
