...
breaks <- seq(-7, 7, length.out = 21)

gplots::heatmap.2(test4, trace = "none", na.color = "black", 
                  col = my_palette, breaks = breaks, distfun = function(x) cluster::daisy(x, metric = "gower"), 
                  hclustfun = function(x) hclust(x))

test8 <- hclust(cluster::daisy(test4, metric = "gower")) %>% 
  cutree(., k = 3)

test9 <- cbind(test4, test8)

test10 <- gplots::heatmap.2(test4, trace = "none", na.color = "black", 
                  col = my_palette, breaks = breaks, distfun = function(x) cluster::daisy(x, metric = "gower"), 
                  hclustfun = function(x) hclust(x))

test11 <- test4[rev(test10$rowInd), test10$colInd]



htmp2(test5, col_limit = 7, plot = TRUE)
