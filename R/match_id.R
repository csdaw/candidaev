#' Create a vector based on matching values with a reference table
#'
#' @description \code{match_id} matches a character vector with values (which may or may not be
...
#'
#' @param new string: name of character column in reference table with
#' corresponding information/values of interest
#'
#' @param concat logical: if \code{TRUE} then if a value in \code{id} has multiple
...
#'
#' @param str_split logical: if \code{TRUE} then values in \code{id} with concatenation,
...
#'
#' @return If \code{concat = TRUE}, returns a vector with the same length as \code{id}
