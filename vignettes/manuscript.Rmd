...
f2b <- nta_bioavg %>% 
  ...
  geom_vline(xintercept = 100, colour = "#990000", linetype = "dashed") +
  geom_line(size = 0.5) + 
...
```{r figure3Data, cache=TRUE, eval=FALSE}
...
```{r figure3, fig.cap='Volcano plots', fig.width=6, fig.height=7, cache=TRUE, eval=FALSE}
ggarrange(f3a1, f3a2,
          f3b1, f3b2,
          f3c1, f3c2,
          f3d1, f3d2,
...
```{r figure4Data}
# figure 4a PCA
f4a_data <- cbind(atcc_de, 
                  biofilm_de[match(rownames(atcc_de), rownames(biofilm_de)), ])

f4a_data <- cbind(f4a_data, 
                  yeast_de[match(rownames(f4a_data), rownames(yeast_de)), ])

colnames(f4a_data)[13:28] <- c("B_EV1", "B_EV2", "B_EV3", "B_EV4", "B_EV5", 
                               "B_W1", "B_W2", "B_W3", "B_W4", "B_W5", 
                               "Y_EV1", "Y_EV2", "Y_EV3", 
                               "Y_W1", "Y_W2", "Y_W3")

f4a_labels <- c(rep("EV", 3), rep("WCL", 3), 
                rep("EV", 3), rep("WCL", 3), 
                rep("EV", 5), rep("WCL", 5), 
                rep("EV", 3), rep("WCL", 3))
  
f4a <- plot_mds(f4a_data, mat_labels = f4a_labels, shape_size = 3) + 
  theme(legend.position = "top") + 
  scale_x_continuous(limits = c(-2.5, 1.5), 
                     expand = c(0,0)) + 
  scale_y_continuous(limits = c(-1.5, 3), 
                     expand = c(0,0)) + 
  scale_colour_manual(breaks = c("A1", "A9", "B", "Y"), 
                      values = c("red", "green", "orange", "blue"), 
                      labels = c("A10231", "A90028", "DAY B", "DAY Y")) + 
  scale_shape_manual(breaks = c("A1", "A9", "B", "Y"), 
                     values = c(15:18), 
                     labels = c("A10231", "A90028", "DAY B", "DAY Y")) + 
  theme_bw() + 
  theme(legend.title = element_blank(),
        legend.position = c(1.7, -0.1), 
        legend.text = element_text(size = 8), 
        legend.direction = "horizontal")
  
# figure 5b 4 way venn
f4b_comp <- list("DAY Y" = rownames(y_ev), 
                 "DAY B" = rownames(b_ev),
                 "ATCC90028" = rownames(atcc9_ev), 
                 "ATCC10231" = rownames(atcc1_ev))
f4b_comp <- lapply(f4b_comp, function(x) match_id(x, ref = uniprot, match = "UP_accession", new = "CGD_gene_name"))

f4b <- plot_venn(vlist = f4b_comp, use_uniprot = FALSE, type = "plot", 
                 fontfamily = "sans", 
                 cat.fontfamily = "sans", 
                 cex = 1, 
                 cat.cex = 0.8, 
...
f4b_overlap <- plot_venn(vlist = f4b_comp, use_uniprot = FALSE, type = "list") %>% 
  filter(..count.. == 405) %>% 
  pull(..values..) %>% 
  unlist()

# f4b_b_excl <- plot_venn(vlist = f4b_comp, use_uniprot = FALSE, type = "list") %>% 
#   filter(..count.. == 116) %>% 
#   pull(..values..) %>% 
#   unlist()
...
```{r figure4, fig.cap='Combined comparison', fig.width=5, fig.height=2.5}
ggarrange(f4a, f4b, 
          labels = c("A", "B"), 
          ncol = 2,
          nrow = 1, 
...
f5_table <- a9_res %>% 
  ...
  filter(CGD_gene_name %in% f4b_overlap)
...
```{r figure6, fig.cap='upset plot', fig.width=3.5, fig.height=3}
upset_data <- marker_comp

names(upset_data) <- c("DAY286 B", "DAY286 Y", "ATCC10231", "ATCC90028")

upset(fromList(upset_data), 
      mainbar.y.label = "\n\n\nIntersection protein count", 
      mainbar.y.max = 150,
      line.size = 1.2, 
      text.scale = c(1.2, 1.3, 1, 1, 1.1, 1.5), 
      sets.x.label = "Total protein number", 
      order.by = "degree", 
      queries = list(list(query = intersects, 
                          params = list("DAY286 B", "DAY286 Y", "ATCC10231", "ATCC90028"), 
                          active = TRUE)))
