...
fS1_cap <- c("\\textbf{Size distribution of \\textit{C. albicans} EVs as determined by 
              nanoparticle tracking analysis (NTA).} A line plot for each EV biological 
              replicate from each strain is shown. Each line plots represents the average 
             of three technical replicates and the dashed grey lines indicate 100 nm.")
...
nta_metadata <- data.frame(id = c("ATCC10231_1", "ATCC10231_2", "ATCC10231_3", 
                           ...
                           strain = c(rep("ATCC10231", 3), 
                                      ...
                                      rep("Biofilm", 5), 
                                      rep("DAY286", 3)), 
                           replicate = c(seq(1, 3), seq(1, 3), seq(1, 5), seq(1, 3)), 
...
nta_total <- nta_techavg %>% 
  ...
  select(strain, replicate, OD600, culture_vol, protein_conc, 
         Particle_count, mean, mode, sd, d10, d50, d90) %>% 
...
tabS1_cap <- "\\textbf{Metadata for each EV isolation analysed in this study.} 
              The optical density (at 600 nm) and volume of each culture from which 
              the EVs were isolated is given. The protein and particle concentration 
              of the EVs along with the NTA size distribution results 
              for each biological replicate are also shown."
...
nta_total %>% 
  ...
  kable_styling(latex_options = c("scale_down", 
                                  "HOLD_position")) %>% 
  add_header_above(c(" " = 5, "NTA particle size distribution (nm)" = 6)) %>% 
  pack_rows("ATCC10231", 1, 3) %>% 
  pack_rows("ATCC90028", 4, 6) %>% 
  pack_rows("DAY286 biofilm", 7, 11) %>% 
  pack_rows("DAY286 yeast", 12, 14)
```
...
wcl_longlist <- wcl_candidates %>% 
  mutate(Function = match_id(Name, uniprot, "CGD_gene_name", "Protein_name", concat = TRUE, str_split = FALSE),
         DAY.Y = match_id(Name, yeast_res, "CGD_gene_name", "logFC", concat = TRUE, str_split = FALSE), 
         DAY.B = match_id(Name, biofilm_res, "CGD_gene_name", "logFC", concat = TRUE, str_split = FALSE), 
         A1 = match_id(Name, a1_res, "CGD_gene_name", "logFC", concat = TRUE, str_split = FALSE), 
         A9 = match_id(Name, a9_res, "CGD_gene_name", "logFC", concat = TRUE, str_split = FALSE), 
...
function_desc <- c(paste0("\\textit{S. cerevisiae} ortholog is ABP1, actin binding protein", footnote_marker_alphabet(1)), 
                   paste0("Putative alcohol dehydrogenase", footnote_marker_alphabet(2)), 
                   paste0("Putative alpha-mannosidase", footnote_marker_alphabet(1)), 
                   "Vacuolar aspartic proteinase", 
                   paste0("Branched chain amino acid aminotransferase", footnote_marker_alphabet(2)), 
                   paste0("Thioredoxin peroxidase", footnote_marker_alphabet(1)), 
                   paste0("Putative 2-enoyl thioester reductase", footnote_marker_alphabet(1)), 
                   paste0("Glycine decarboxylase P subunit", footnote_marker_alphabet(1)), 
                   "Glycerol 2-dehydrogenase", 
                   paste0("Putative glycogen debranching enzyme", footnote_marker_alphabet(2)), 
                   paste0("Glutathione reductase", footnote_marker_alphabet(1)), 
                   "Glutathione-independent glyoxalase", 
                   "Phosphoglycerate mutase", 
                   "NADH-linked methylglyoxal oxidoreductase", 
                   paste0("Aspartate-semialdehyde dehydrogenase", footnote_marker_alphabet(2)), 
                   "Heat shock protein",
                   paste0("Isocitrate dehydrogenase subunit", footnote_marker_alphabet(1)), 
                   paste0("Isocitrate dehydrogenase subunit", footnote_marker_alphabet(1)), 
                   paste0("Isocitrate dehydrogenase", footnote_marker_alphabet(1)), 
                   paste0("Ketol-acid reductoisomerase", footnote_marker_alphabet(1)), 
                   "Inorganic pyrophosphatase", 
                   paste0("Putative dihydrolipoamide S-succinyltransferase", footnote_marker_alphabet(1)), 
                   paste0("\\textit{S. cerevisiae} ortholog is LHP1, RNA binding protein", footnote_marker_alphabet(1)), 
                   "Dihydrolipoamide dehydrogenase", 
                   paste0("NADH-cytochrome-b5 reductase", footnote_marker_alphabet(1)), 
                   "O-acetylhomoserine O-acetylserine sulfhydrylase", 
                   paste0("Non-histone chromosomal protein 6", footnote_marker_alphabet(1)), 
                   paste0("\\textit{S. cerevisiae} ortholog is NIF3, mitochondrial protein", footnote_marker_alphabet(1)), 
                   paste0("\\textit{S. cerevisiae} ortholog is LPX1, peroxisomal lipase", footnote_marker_alphabet(2)), 
                   paste0("Putative phosphoglycerate mutase family protein", footnote_marker_alphabet(2)), 
                   paste0("Protein of unknown function", footnote_marker_alphabet(3)), 
                   paste0("\\textit{S. cerevisiae} ortholog is PRE7, 20S proteasome subunit (beta6)", footnote_marker_alphabet(1)), 
                   paste0("Predicted dienelactone hydrolase domain", footnote_marker_alphabet(2)), 
                   paste0("20S proteasome subunit (beta7)", footnote_marker_alphabet(1)), 
                   paste0("Putative phosphatidyl synthase", footnote_marker_alphabet(2)), 
                   paste0("Stationary phase enriched protein", footnote_marker_alphabet(2)), 
                   "Putative thiamine biosynthesis enzyme", 
                   paste0("\\textit{S. cerevisiae} ortholog is STM1, regulates translation", footnote_marker_alphabet(1)), 
                   paste0("\\textit{S. cerevisiae} ortholog is NAS6, proteasome chaperone", footnote_marker_alphabet(1)), 
                   paste0("Putative esterase", footnote_marker_alphabet(1)), 
                   "Putative phosphomutase", 
                   paste0("\\textit{S. cerevisiae} ortholog is HSP10, mitochondrial co-chaperonin", footnote_marker_alphabet(1)), 
                   paste0("Putative X-Pro aminopeptidase", footnote_marker_alphabet(2)), 
                   paste0("NADPH oxidoreductase family protein", footnote_marker_alphabet(2)), 
                   paste0("Pyridoxamine-phosphate oxidase", footnote_marker_alphabet(1)), 
                   paste0("Glucose-6-phosphate isomerase", footnote_marker_alphabet(2)), 
                   paste0("20S proteasome subunit (beta2)", footnote_marker_alphabet(1)), 
                   "20S proteasome subunit (alpha5)", 
                   paste0("Putative single-stranded DNA-binding protein", footnote_marker_alphabet(1)), 
                   paste0("\\textit{S. cerevisiae} ortholog is SBP1, eIF4G binding protein", footnote_marker_alphabet(1)), 
                   paste0("Proteasome subunit YC7alpha", footnote_marker_alphabet(1)), 
                   paste0("Putative small tetratricopeptide repeat-containing protein", footnote_marker_alphabet(1)), 
                   "Superoxide dismutase", 
                   paste0("HSP90 co-chaperone", footnote_marker_alphabet(1)), 
                   paste0("Translation initiation factor eIF1a", footnote_marker_alphabet(1)), 
                   "\\textit{S. cerevisiae} ortholog is TMA19, ribosome associated protein", 
                   paste0("Predicted succinate semialdehyde dehydrogenase", footnote_marker_alphabet(1)), 
                   paste0("Dihydroorotase", footnote_marker_alphabet(2)), 
                   paste0("Putative xylulokinase", footnote_marker_alphabet(2)), 
                   "Nucleoside disphosphate kinase")

wcl_longlist <- wcl_longlist %>% 
  mutate(Function = function_desc, 
         VDS = c(rep("", 60)), 
         pack_row = c(8, 1, rep(4, 2), 1, 3, rep(2, 2), 7, rep(1, 2), rep(7, 3), 1, 7, 
                      rep(2, 4), 7, 2, 3, rep(2, 2), 7, 3, 2, 5, rep(1, 2), 6, 
                      2, 6, 2, 1, 7, rep(1, 3), 7, 2, 1, 5, 2, 1, rep(6, 2), 
                      2, 1, 6, 1, 2, 1, 9, 7, rep(1, 4))) %>% 
  arrange(pack_row)

# fix MCR1 DAY B log2(fc) decimal points
wcl_longlist[29, 4] <- "-0.95"

# HSP12;orf19.4216 label too long, fix.
wcl_longlist[53, 1] <- "HSP12"
...
```{r tableS2}
tabs2_cap <- "\\textbf{Candidate negative protein markers for \\textit{C. albicans} EVs.} 
             This list of proteins consists of those that were found to be exclusive to 
             whole cell lysate (WCL) or 
             significantly enriched in WCL across the four \\textit{C. albicans} strains examined in 
             ...
             WCL for each strain is listed. A negative value indicates that a protein 
             was enriched in WCL compared to EV. \`\`ex\" indicates where a protein was only 
             identified in the WCL fraction and not in EVs for that strain. The \`\`TM\" 
             ...
             (ref:table1). Underlined proteins are those identified as the best candidates for negative
...
knitr::kable(wcl_longlist[, -10], 
             ...
             linesep = "", 
             escape = FALSE) %>% 
  row_spec(row = c(28, 36, 41, 51, 53, 59), underline = TRUE) %>% 
  add_header_above(c(" " = 2, "log\\\\textsubscript{2}(fold change) EV vs WCL" = 4, " " = 2), escape = FALSE) %>% 
  kable_styling(font_size = 8, 
                latex_options = c("HOLD_position")) %>% 
  pack_rows("Cytoplasm", 1, 20) %>% 
  pack_rows("Mitochondria", 21, 36) %>% 
  pack_rows("Nucleus", 37, 39) %>% 
  pack_rows("Vacuole", 40, 41) %>% 
  pack_rows("Peroxisome", 42, 43) %>% 
  pack_rows("Proteasome", 44, 48) %>% 
  pack_rows("Cell wall, cell surface, fungal biofilm matrix", 49, 58) %>% 
  pack_rows("Cytoskeleton", 59, 59) %>% 
  pack_rows("Translation preinitiation complex", 60, 60) %>% 
  footnote(alphabet = c("Protein localisation was inferred from sequence similarity with 
                        ...
                        "Protein localisation was predicted using BUSCA (ref:footnote3)"), 
           escape = FALSE, 
           threeparttable = TRUE)
```
